...
impl UIContext {
    ...
    pub fn resize_window(&self, width: u32, height: u32) {
        let header_h = self.header_bar.height();
        if width > 0 && height > 0 {
            let factor: f64 = width as f64 / height as f64;
            if height >= width {
                let w: f64 = self.window.width_request() as f64;
                self.window.set_default_size(w as i32, (w / factor) as i32 + header_h);
            }
            else {
                let h: f64 = self.window.height_request() as f64;
                self.window.set_default_size((h * factor) as i32, h as i32 + header_h);
            }
        }
        else {
            self.window.set_default_size(self.window.width_request(), self.window.width_request() + header_h);
        }
    }
